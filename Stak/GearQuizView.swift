...
struct GearQuizView: View {
    @StateObject var viewModel = GearQuizViewModel()
    @State private var showResult: Bool = false
    
    var body: some View {
        ZStack {
            Color(red: 41/255, green: 128/255, blue: 185/255).ignoresSafeArea()
...
struct QuizContentView: View {
    @ObservedObject var viewModel: GearQuizViewModel
    var onFinish: () -> Void
    var body: some View {
        VStack(spacing: 0) {
            VStack(spacing: 0) {
                // 1. Subtitle всегда
                Text("Wash like a Pro")
                            ...
                            HStack(spacing: 10) {
                                ForEach(0..<4) { j in
                                    ...
                                    if idx < viewModel.totalQuestions {
                                        ZStack {
                                            Circle()
                                                .fill(idx == viewModel.currentIndex ? Color(red: 52/255, green: 152/255, blue: 219/255) : Color(red: 44/255, green: 62/255, blue: 80/255))
            ...
            Button(action: {
                ...
                .padding()
                .background(viewModel.isAnswered ? Color(red: 52/255, green: 152/255, blue: 219/255) : Color(red: 44/255, green: 62/255, blue: 80/255))
                .cornerRadius(12)
            }
            .padding(.horizontal, 20)
            .padding(.bottom, 84)
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 12) {
                ZStack {
                    Circle()
                        .fill(state == .wrong && isSelected ? Color.red : Color(red: 44/255, green: 62/255, blue: 80/255))
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    .stroke(
                        isSelected ? (state == .correct ? Color(red: 52/255, green: 152/255, blue: 219/255) : Color.red) : Color.white.opacity(0.1),
                        lineWidth: 2
                    )
                    .background(
                        RoundedRectangle(cornerRadius: 12)
                            .fill(isSelected && state == .wrong ? Color.red.opacity(0.15) : Color(red: 44/255, green: 62/255, blue: 80/255))
                    )
            )
        }
        .disabled(state != .none)
                ...
                ZStack {
                    ...
                    VStack(spacing: 2) {
                        Text("\(viewModel.correctCount())/16")
                            .PoppinsBold(size: 32, color: Color(red: 52/255, green: 152/255, blue: 219/255))
                        Text("Outstanding Score!")
                            .PoppinsBold(size: 16, color: .white)
                        Text("You're a true gear care expert")
                            .Poppins(size: 13, color: .white.opacity(0.8))
                    }
                    .padding(16)
                }
                ...
                VStack(spacing: 10) {
                    ForEach(viewModel.topics) { topic in
                        HStack(spacing: 14) {
                            ...
                            Text("\(score)/4")
                                .PoppinsBold(size: 15, color: score == 4 ? Color(red: 52/255, green: 152/255, blue: 219/255) : .red)
                        }
                        .padding(10)
