...
class GearQuizViewModel: ObservableObject {
    @Published var questions: [GearQuizQuestion] = []
    ...
    func startQuiz() {
        questions = GearQuizModel.allQuestions.shuffled().prefix(totalQuestions).map { $0 }
        currentIndex = 0
...
struct GearQuizTopic: Identifiable {
    let id = UUID()
    let number: Int
    let title: String
    let subtitle: String
...
extension GearQuizViewModel {
    // Темы жёстко по 4 вопроса, как на макете
    var topics: [GearQuizTopic] {
        [
            .init(number: 1, title: "Wash like a Pro", subtitle: "Equipment washing", questionRange: 0..<4),
    ...
    func scoreForTopic(_ topic: GearQuizTopic) -> Int {
